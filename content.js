class PureNotepad {
  ...
  applyColorTheme(colorIndex) {
    ...
    if (color) {
      // Don't apply background to body, only to the notepad container
...
// Prevent multiple instances and script re-injection
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.action === 'ping') {
    if (window.pureNotepadInstance) {
      ...
      sendResponse({success: false, initialized: false});
    }
    return true;
  } else if (request.action === 'toggleNotepad') {
    if (!window.pureNotepadInstance) {
